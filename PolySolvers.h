...
namespace PolySolvers
{
  ...
  inline std::vector<P> SolveQuadratic(P a, P b, P c)
  {
    ...
    if (delta < 0) return std::vector<P>();
    if (a == 0)
    {
  ...
  inline std::vector<P> SolveQuartic( P a, P b, P c, P d, P e)
  {
    ...
    if (n == 0) return solution3;

    r0 = solution3[0];
    /* calculate R^2 */
    R2 = 0.25 * b2 - c + r0;
    if (R2 < 0) return std::vector<P>();
    ...
    switch (nb_real_roots)
    {
        ...
        case 4:
            ...
            //break;
        default: return std::vector<P>(); // just to shut the compiler up....
            //break;
    }
