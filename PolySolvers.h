...
namespace PolySolvers
{
    ...
    inline std::vector<P> SolveCubic(P a, P b, P c, P d)
    {
        ...
        Q = (3 * c_a - b_a2) / 9.0;
        R = (9 * b_a * c_a - 27 * d_a - 2 * b_a * b_a2) / 54.0;
        Q3 = Q * Q * Q;
        D = Q3 + R * R;
        b_a_3 = (1.0 / 3.0) * b_a;

        // Fix 1: EPS check to prevent division by zero and sqrt(negative) later
        if (std::abs(Q) < EPS)
        {
            if (std::abs(R) < EPS)
            {
            ...
            {
                // Fix 2: Use std::cbrt instead of pow
        ...
        {
            /* three real roots */
            // Fix 3: Clamp the ratio to [-1.0, 1.0] to prevent acos(NaN)
            P ratio = R / std::sqrt(-Q3);
            ratio = std::max<P>(-1.0, std::min<P>(1.0, ratio));
        ...
        // Fix 4: Safely use std::cbrt to handle negative numbers natively
        AD = std::cbrt(R + std::sqrt(D));
        BD = (std::abs(AD) < EPS) ? 0.0 : -Q / AD;
